<table class="uk-table uk-table-striped uk-text-extralarge uk-height-1-1">
    ...
    foreach ($orderMainArchive as $orderMain): ?>
        <tr class="uk-margin-small <?php if ($orderMain->OrderState == 2) echo('uk-alert uk-alert-extradanger') ?>"
            ...
            <td><?php if (!is_null($orderMain->user)) echo h($orderMain->user->FullName) ?></td>
...
<?php
//echo $this->element('pagination',["modelName" => 'OrderMainArchive','options'=>['ajaxPagination'=>'true','paginationContainer'=>'#archive-container','urlParams'=>'&userID='.$loggedUser->ID.'&restaurantID='.$restaurant->ID]]);
?>